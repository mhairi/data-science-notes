...
# Order of Units One and Two

Unit's one and two can be done in either order. Very roughly this unit covers inferential statistics and the second unit covers working with data in R.  Indeed it may be a good idea to work on both units simultanously as they are using quite different skills. If you find you are struggling with the maths in this first unit switch to learning programming in the second unit. If you want to do less programming switch from the second unit to the first. 

This unit also covers the very bare-bones of what is needed to do statistics in R. Because the units can be done in either order there is some repetition in the R content so certain parts of are included as supplamentary chapters. These chapters can be done at the relavent time when working though either unit - I'll let you know when you need to do them. Once you've worked though one unit there's no need to compleate the supplamentary chapter again. Although, if you struggled with the material the first time round and think some repistion would help please try the chapter again.
...
At this point, if you haven't already done so, please go though the supplamentary unit 1 on using R. We're about to get stuck in with analysing some data in R and if you havne't covered the basics of R you'll soon be stuck.
...
Open up R and RStudio. Create a vector in R containing the data above. Assign the name `spinach_rating` to it. 
...
Before continuing make sure you can done the supplamentary chapter on data frames in R. We will be using the spinach data-frame that we read in in that chapter.
